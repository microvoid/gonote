...
export class MarkdownExtension extends PlainExtension<MarkdownOptions> {
  ...
  onCreate(): void {
    this.store.setStringHandler(
      "markdown",
      this.markdownToProsemirrorNode.bind(this)
    );
  }
  ...
  private markdownToProsemirrorNode(
    ...
    return this.store.stringHandlers.html({
      ...
      content: this.options.markdownToHtml(
        options.content,
  ...
  insertMarkdown(
    ...
    return props => {
      const { state } = props;
      let html = this.options.markdownToHtml(
        markdown,
        this.options.htmlSanitizer
      );

      html =
        !options?.alwaysWrapInBlock &&
        html.startsWith("<p><") &&
        html.endsWith("</p>\n")
          ? html.slice(3, -5)
          : `<div>${html}</div>`;
